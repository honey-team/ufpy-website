...
!!! tip "You can define `default` only for this calling"
    ```py
...
!!! tip "You can use indexes and slices"
    ```python
    d[1] = 2
    d[2:6:2] = 8
    d[:3] = 1, 2, 3
...
!!! example "You can get length of dict using inbuilt `len()` function"
    ```python
    d = UDict(hello=1, hi=2)
    len(d) # 2
    ```

### Iterate dict: `keys`, `values`, `items` properties {data-toc-label="Iterate dict"}

!!! example "You can iterate dict using `for key, value in UDict` syntax."
    ```python
    d = UDict(hello=1, hi=2)
    for key, value in d:
        print(key, value)
...
!!! tip "Also, you can iterate `items` property for this result"
    ```python
    for key, value in d.items:
        ...
    ```

!!! tip "If you want iterate only all keys or values, use `keys` or `values` properties"
    ```python
    for key in d.keys:
        ...
...
!!! example "You can use `is_empty()` method to check that UDict is empty"
    ```python
    d = UDict()
...
!!! tip "You also can use `#!py if UDict` or `#!py bool(UDict)` syntax"
    ```python
    d = UDict()
    print(bool(d)) # False
...
!!! example "You can check that key in dict"
    ```python
...
!!! tip "You can also check that item in dict"
    ```python
    d = UDict(hi=1, hello=2)
...
### Using `#!py repr()` and `#!py print()`

!!! example "Because `#!py print()` uses `#!py repr()`, you can directly print `UDict` objects"
    ```python
    d = UDict(hi=1, hello=2)
...
!!! example "You can compare `UDict`s using inbuilt compare operators (`==`, `!=`, `>`, `>=`, `<`, `<=`)"
    !!! note
...
!!! example "You can use inbuilt math operators (`+`, `-`, `*`, `/`, `+=`, `-=`, `*=`, `/=`) with `UDict`s"
...
!!! example "You can use unary minus with dicts"
    ```python
    d = UDict(hi=1, hello=2)
    print(-d) # u{'hi': -1, 'hello': -2}
    ```
...
!!! example "You can reverse dict using `reverse()` or `reversed()` method"
    !!! danger "When you use `reverse()`, dict updates in contrast to `reversed()`. Be careful!"
        ```python
        # reverse()
        d = UDict(b=1, a=2)
...
!!! tip "You can use `~` operator and `#!py reversed()` class. They are equivalents of `UDict.reversed()`"
    ```python
    ~d
    reversed(d)
    ```
...
!!! example "You can use `sort()` and `sorted()` methods for sorting UDict"
    !!! danger "`sort()` is updating dict, `sorted()` - isn't. Be careful!"
        ```python
        # sort()
        d = UDict(b=1, a=2)
